...
import com.coffee.sales.repository.UserRoleRepository;
...
public class BaseUserService extends AbstractService {
	...
	public String setRolesToUser(int idUser, int idRole) {
		...
		userRoles.setUserId(baseUser.getId());
	...
	public String deleteRolesToUser(int idUser, int idRole) {
		...
		UserRoles userRoles = findByUserIdAndRoleId(baseUser.getId(), baseRole.getId());
	...
	private void existRole(BaseRole baseRole, BaseUser baseUser) {
		...
		Optional<UserRoles> optUserRoles = userRoleRepository.findByUserIdAndRoleId(baseUser.getId(),
				baseRole.getId());
